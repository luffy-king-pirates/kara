...
<head>
    <meta charset="UTF-8">
    <title>credit Transaction #{{ $credit->id }}</title>
    <style>
        body {
...
<body>
    <div class="header">
        ...
        <p>Date: {{ $credit->creation_date }} | Invoice # {{ $credit->id }}</p>
    </div>

    <div class="container">
        <p><strong>Company Name:</strong> {{ $credit->customer->name }}</p>
        <p><strong>TIN Number:</strong> {{ $credit->customer->tin_number }}</p>
        <p><strong>VRN Number:</strong> {{ $credit->customer->vrn_number }}</p>
        ...
        <table class="details">
            ...
            <tbody>
                @foreach ($credit->details as $index => $detail)
                    <tr>
                        <td>{{ $index + 1 }}</td>
        ...
        <table class="summary-table">
            <tr>
                <th>Subtotal:</th>
                <td>{{ number_format($credit->details->sum('total'), 2) }} TZS</td>
            </tr>
            <tr>
                <th>VAT (18%):</th>
                <td>{{ number_format($credit->details->sum('total') * 0.18, 2) }} TZS</td>
            </tr>
            <tr class="bold">
                <th>Total (TZS):</th>
                <td>{{ number_format($credit->total_amount, 2) }} TZS</td>
            </tr>
        </table>
