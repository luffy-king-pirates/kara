...
<head>
    <meta charset="UTF-8">
    <title>proforma Transaction #{{ $proforma->id }}</title>
    <style>
        body {
...
<body>
    <div class="header">
        ...
        <p>Date: {{ $proforma->creation_date }} | Invoice # {{ $proforma->id }}</p>
    </div>

    <div class="container">
        <p><strong>Company Name:</strong> {{ $proforma->customer->name }}</p>
        <p><strong>TIN Number:</strong> {{ $proforma->customer->tin_number }}</p>
        <p><strong>VRN Number:</strong> {{ $proforma->customer->vrn_number }}</p>
        ...
        <table class="summary-table">
            <tr>
                <th>Subtotal:</th>
                <td>{{ number_format($proforma->details->sum('total'), 2) }} TZS</td>
            </tr>
            <tr>
                <th>VAT (18%):</th>
                <td>{{ number_format($proforma->details->sum('total') * 0.18, 2) }} TZS</td>
            </tr>
            <tr class="bold">
                <th>Total (TZS):</th>
                <td>{{ number_format($proforma->total_amount, 2) }} TZS</td>
            </tr>
        </table>
