...
@php
    ...
    $totalItems = $proforma->details->count();
    $numIterations = $totalItems > 21 ? ceil($totalItems / 21) : 1; // Calculate number of iterations
    $itemsPerIteration = $numIterations === 1 ? ($headers === 'false' ? 19 : 13) : ($headers === 'false' ? 25 : 21);
...
@for ($i = 0; $i < $numIterations; $i++)
    @php
        ...
        $slicedDetails = $proforma->details->slice($start, $itemsPerIteration);
                    ...
                    <tr class="h10address">
                        ...
                        <td class="value">
                            @php
                                ...
                                echo \Carbon\Carbon::parse($proforma->creation_date)->format('M d, Y');
                            @endphp
                        </td>
                    </tr>
                    <tr class="h10address">
                        ...
                        <td style="font-size: 9pt;width: 30%; text-align: center;font-family:Calibri, sans-serif; border: .5px solid gray;line-height: 20px;"
                            ...
                            <b>TIN NUMBER: {{ $proforma->customer->customer_tin }}<br>VRN NUMBER:
                                {{ $proforma->customer->customer_vrn }}</b>
                        </td>
                        <td class="label">PROFORMA INVOICE#</td>
                        <td class="value">{{ $proforma->proforma_number }}</td>
                    </tr>
                    <tr class="h10address">
                ...
                <tbody>
                    <tr class="h10details">
                        ...
                        <td colspan="4" class="value-left" style="text-transform: uppercase;">
                            {{ $proforma->customer->customer_name }}</td>
                    </tr>
                    <tr class="h10details">
                        <td class="label-left-details">TIN Number</td>
                        <td width="15%" class="text-center"><b>:</b></td>
                        <td colspan="4" class="value-left">{{ $proforma->customer->customer_tin }}</td>
                    </tr>
                    <tr class="h10details">
                        <td class="label-left-details">VRN Number</td>
                        <td width="15%" class="text-center"><b>:</b></td>
                        <td colspan="4" class="value-left">{{ $proforma->customer->customer_vrn }}</td>
                    </tr>
                </tbody>
            </table>
        @endif
            ...
            <tbody>
                ...
                @if ($end === $totalItems - 1)
                    ...
                    <tr>
                        ...
                        <td class="x19" style="text-align: right"><b>{{ $proforma->total_amount }}</b></td>
                    </tr>
                    <tr>
