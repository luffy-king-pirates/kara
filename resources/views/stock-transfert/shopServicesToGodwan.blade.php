...
@section('title', 'Shop (Service) to Godown')

@section('content_header')
    <h1>Shop (Service) to Godown</h1>
@stop
            ...
            <div class="row">
                ...
                <div class="col-md-3">
                    <div class="form-group">
                        ...
                        <input type="text" class="form-control" id="transfer_from" value="Shop (Service)" disabled>
                    </div>
                </div>
                <div class="col-md-3">
                    <div class="form-group">
                        <label for="transfer_to">Transfer To</label>
                        <input type="text" class="form-control" id="transfer_to" value="Godown" disabled>
                    </div>
                </div>
            </div>
        </form>
        ...
        function addRow() {
            let newRow = `
                <tr>
                    ...
                    <td><input type="text" class="form-control" value="Shop (Service)" disabled></td>
        ...
        function initializeAutocomplete(element) {
            $(element).autocomplete({
                ...
                select: function(event, ui) {
                    // Fetch Shop, Godown, and Unit values based on selected item
                    const selectedItem = ui.item.value;

                    // Replace with actual logic to fetch item details
                    const shopValue = `Shop (Service) for ${selectedItem}`;
                    const godownValue = `Godown for ${selectedItem}`;
                    const unitValue = `Unit for ${selectedItem}`;

                    $(this).closest('tr').find('input[disabled]').eq(0).val(shopValue);
                    $(this).closest('tr').find('input[disabled]').eq(1).val(godownValue);
