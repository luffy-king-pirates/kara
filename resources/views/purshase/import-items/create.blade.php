...
@section('title', $localpurshase ? 'Edit localpurshase Transaction' : 'Create localpurshase Transaction')

@section('content_header')
    <h1>{{ $localpurshase ? 'Edit' : 'Create' }} localpurshase Transaction</h1>
@stop

@section('content')
    <div class="container">
        <form id="localpurshase_form" method="POST" action="{{ $localpurshase ? route('localpurshase.update', $localpurshase->id) : route('localpurshase.store') }}">
            @csrf
            @if ($localpurshase)
                @method('PUT')
            @endif
            ...
            <div class="row mb-3">
                <div class="col-md-4">
                    <div class="form-group">
                        <label for="localpurshase_number">purshase Number</label>
                        <input type="text" class="form-control" id="localpurshase_number" name="localpurshase_number"
                            value="{{ $localpurshase ? $localpurshase->localpurshase_number : old('localpurshase_number') }}" placeholder="Enter localpurshase Number"
                            required>
                    </div>
                </div>
                <div class="col-md-4" id="customer_name_div">
                    <div class="form-group">
                       <label for="supplier_autocomplete">Supplier</label>
                        <input type="text" class="form-control" id="supplier_autocomplete" name="supplier_name" placeholder="Select Supplier" required>
                ...
                <div class="col-md-4">
                    <div class="form-group">
                        <label for="creation_date">Purshase Date</label>
                        <input type="text" class="form-control" id="creation_date" name="creation_date"
                            value="{{ $localpurshase ? $localpurshase->creation_date : \Carbon\Carbon::now()->toDateString() }}" readonly>
                    </div>
                </div>
            ...
            <table class="table table-bordered" id="localpurshase_table">
                <thead>
                    <tr>
                        <th>S/N</th>
                        <th>Item Name</th>
                ...
                <tbody>
                    @if ($localpurshase && $localpurshase->details->count())
                        @foreach ($localpurshase->details as $detail)
                            <tr>
                    ...
                    <tr>
                        ...
                        <th>
                            <input type="number" class="form-control" id="total_amount_table" name="total_amount_table"
                                value="{{ $localpurshase ? $localpurshase->details->sum('total') : 0 }}" disabled>
                        </th>
                    </tr>
                </tfoot>
            </table>
            ...
            <div class="text-right mb-3">
                <a href="{{ route('localpurshase.index') }}" class="btn btn-danger">Discard</a>
        ...
        <div class="toast-container position-fixed bottom-0 end-0 p-3" style="z-index: 11;">
            <div id="successToast" class="toast align-items-center text-white bg-success border-0" role="alert"
                ...
                <div class="d-flex">
                    <div class="toast-body">localpurshase transaction saved successfully!</div>
...
@section('js')
    ...
    <script>
        let rowIndex = {{ $localpurshase ? $localpurshase->details->count() + 1 : 1 }};
        let totalQuantity = 0;
        ...
        function calculateTotals() {
            ...
            $('#localpurshase_table tbody tr').each(function() {
 ...
 <td>
        ...
        `;
            ...
            $('#localpurshase_table tbody').append(newRow);
            rowIndex++;
            calculateTotals();

            // Update total quantity after adding a new row
            updateTotalQuantity();
            initializeAutocomplete($('#localpurshase_table tbody tr:last .item-name'));
        }
        ...
        $(document).ready(function() {
            initializeAutocomplete($('#localpurshase_table tbody .item-name'));
            $('.toast').toast({
                autohide: true,
                delay: 3000
        ...
        $('#save_btn').click(function() {
            ...
            const formData = $('#localpurshase_form').serialize();
            $.ajax({
                url: $('#localpurshase_form').attr('action'),
                method: 'POST',
                data: formData,
                success: function(response) {
                    $('#successToast').toast('show');
                    window.location.href = "{{ route('localpurshase.index') }}"; // Redirect after success
                },
                error: function(xhr) {
